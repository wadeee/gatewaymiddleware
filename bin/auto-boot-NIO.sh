...
scp -P ${remote_port} -i ${ssh_key} ../released/NIOServer.jar ${remote_user}@"${remote_host}":/root/jars/NIOServer.jar

## upload config
scp -P ${remote_port} -i ${ssh_key} ../config/NIOServer.service ${remote_user}@"${remote_host}":/usr/lib/systemd/system/

## restart service
ssh -p ${remote_port} -i ${ssh_key} ${remote_user}@"${remote_host}" "systemctl daemon-reload"
ssh -p ${remote_port} -i ${ssh_key} ${remote_user}@"${remote_host}" "systemctl enable NIOServer"
ssh -p ${remote_port} -i ${ssh_key} ${remote_user}@"${remote_host}" "systemctl restart NIOServer"
